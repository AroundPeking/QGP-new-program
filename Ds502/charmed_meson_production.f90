...
!    CHISQ = 0.0
    ...
    !open(unit = 11, file = "dNptdpt_Jpsi_c020_502.dat", status = "unknown")
    !write(11, 108)
    !phigh=10.00
    !plow=0.2
    !N=30
    ...
    !---------------------------------calculate D0 begin---------------------------------------------
    !open(unit = 18, file = "dNptdpt_D0_c020_502.dat", status = "unknown")
    !write(18, 108)
    !phigh=21.00
    !plow=0.2
    !N=50
    ...
    !---------------------------------calculate Ds begin---------------------------------------------
    open(unit = 22, file = "dNptdpt_Ds_c020_502.dat", status = "unknown")
    write(22, 108)
    phigh=21.00
    plow=0.2
    N=50
...
! c, cbar quark by linear relation between fik and sNN, interpolating between 5.5 and 0.2 TeV.
   ...
   !        /(k*(1 + 16/(0.1 + k)**2)))!fik vs s_NN
    ...
    !          39.372261643524276/k**5.5)/       &
    !        (k + (16.*k)/(0.1 + k)**2))**0.2*   &
    !     (16.01 + k*(0.2 + k)))!0.2 weight=0.2     5.5 weight=0.8
        
        fik = Exp(20.49 - 3.15879* k**0.5)/(3.205558* 10**6)!FONLL fit
